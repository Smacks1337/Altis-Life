...
if(_isVehicle && _curTarget in life_vehicles) exitWith {hint "Du hast bereits die Schluessel zu diesem Fahrzeug."};
...
if(!isNil "_badDistance") exitWith {titleText["Du hast dich zu weit von deinem Ziel entfernt.","PLAIN"]; life_action_inUse = false;};
...
if(!_isVehicle) then {
	...
	_dice = random(100);
	if(_dice < 30) then {
		titleText["Du hast nun die Schluessel zu diesem Fahrzeug.","PLAIN"];
		...
		titleText["Der Dietrich ist kaputt gegangen.","PLAIN"];
	};
};